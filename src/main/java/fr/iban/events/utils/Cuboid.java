...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
	...
	public static Cuboid getFromWorldEdit(Player player) {
		WorldEditPlugin worldEdit = EventsPlugin.getInstance().getWorldEditPlugin();
		if(worldEdit != null) {
			try {
				World world = player.getWorld();
				BlockVector3 pos1 = worldEdit.getSession(player).getSelection(BukkitAdapter.adapt(world)).getMinimumPoint();
				BlockVector3 pos2 = worldEdit.getSession(player).getSelection(BukkitAdapter.adapt(world)).getMaximumPoint();
